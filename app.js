...
let state = {
    ...
    dataStorageFiles: [], // { id, category, nik, empName, fileName, fileData, uploadDate, trainingName, date, trainer, model, level, result, status }
    cloudSettings: {
        enabled: false,
        apiKey: '',
        projectId: '',
...
const addDataStorageModal = new bootstrap.Modal(document.getElementById('addDataStorageModal'));
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        if (state.cloudSettings && state.cloudSettings.enabled) {
            initCloudSync();
        }
    } catch (err) {
        console.error('Failed to initialize:', err);
        alert('Gagal memuat database! Silakan coba hapus cache browser atau jalankan di mode private. Error: ' + err.message);
    }
});
...
if (loginForm) {
    loginForm.addEventListener('submit', async (e) => {
        ...
        loginBtn.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i> Mohon Tunggu...';
        ...
        if (u.toUpperCase() === 'PPDSHE' && p === 'ppdshe2026') {
...
function checkAuth() {
    if (state.user) {
        ...
        if (loginSection) {
            ...
            loginSection.style.display = 'flex'; // Ensure flex is restored
        }
...
async function initCloudSync() {
    if (!state.cloudSettings || !state.cloudSettings.enabled || !state.cloudSettings.apiKey || !state.cloudSettings.projectId) {
        return;
    }

    const config = {
        apiKey: state.cloudSettings.apiKey,
        projectId: state.cloudSettings.projectId,
        databaseURL: state.cloudSettings.dbUrl
    };
    ...
    try {
        ...
        if(cloudSyncStatus) cloudSyncStatus.classList.remove('d-none');
        if(syncStatusText) syncStatusText.innerText = 'Tersinkron';

        // Listen for all changes from cloud
        const dbRef = firebaseDb.ref('hr_data');
        dbRef.on('value', async (snapshot) => {
            if (isSyncingFromCloud) return; // Prevent loop
            
            const data = snapshot.val();
            if (data) {
                isSyncingFromCloud = true;
                try {
                    // Update local state and IndexedDB with cloud data
                    for (const storeName in data) {
                        const storeData = data[storeName];
                        if (Array.isArray(storeData)) {
                            // Update IndexedDB
                            for (const item of storeData) {
                                await dbPutLocalOnly(storeName, item);
                            }
                        } else if (typeof storeData === 'object') {
                            // Single items (like settings)
                            for (const key in storeData) {
                                await dbPutLocalOnly(storeName, storeData[key]);
                            }
                        }
                    }
                    // Refresh app state after pull
                    await loadStateFromDB();
                    renderDashboard();
                    if(syncStatusText) syncStatusText.innerText = 'Data Cloud Terbaru';
                } catch (e) {
        ...
        const syncStatusText = document.getElementById('syncStatusText');
        if(syncStatusText) syncStatusText.innerText = 'Koneksi Gagal';
    }
}

// Local-only put to prevent loops during sync
function dbPutLocalOnly(storeName, data) {
...
// Local-only delete to prevent loops during sync
function dbDeleteLocalOnly(storeName, key) {
    return new Promise((resolve, reject) => {
        const transaction = db.transaction([storeName], 'readwrite');
        const store = transaction.objectStore(storeName);
...
async function pushToCloud(storeName, data, isDelete = false) {
    if (!firebaseDb || isSyncingFromCloud) return;

    try {
        const ref = firebaseDb.ref(`hr_data/${storeName}`);
        
        // Find keyPath for the store to determine ID
        let keyId = data.id || data.nik || data.empId || data.model || data.key;
        if (!keyId && typeof data === 'string') keyId = data;

        if (isDelete) {
            await ref.child(keyId).remove();
        } else {
            // For large files (PDFs), we check size before pushing (Limit 2MB for Firebase)
            if (data.fileData && data.fileData.length > 2000000) {
                console.warn('File too large for cloud sync - metadata only will be synced.');
                // Push metadata but omit fileData to save cloud space/bandwidth
                const metaOnly = { ...data, fileData: 'CLOUD_SYNC_METADATA_ONLY' };
                await ref.child(keyId).set(metaOnly);
            } else {
                await ref.child(keyId).set(data);
            }
        }
    } catch (e) {
...
function initDB() {
    return new Promise((resolve, reject) => {
        ...
        request.onupgradeneeded = (event) => {
            ...
            if (!db.objectStoreNames.contains('data_storage_files')) db.createObjectStore('data_storage_files', { keyPath: 'id' });
        };
...
function dbPut(storeName, data) {
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = async () => {
            ...
            await pushToCloud(storeName, data);
...
function dbDelete(storeName, key) {
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = async () => {
            ...
            await pushToCloud(storeName, key, true);
            resolve();
        };
        request.onerror = (e) => reject(e);
    });
    ...
    try {
        ...
        if (sheetSetting) {
            if (sheetSetting.value.includes('/d/e/export')) {
                console.error('Detected broken URL in DB, resetting to default.');
        ...
        // Load Cloud Settings
        try {
            const cs = await dbGet('settings', 'cloudSettings');
            if (cs) {
                state.cloudSettings = cs.value;
                // Update UI fields
                const switchEl = document.getElementById('cloudSyncSwitch');
                if(switchEl) switchEl.checked = state.cloudSettings.enabled;
                if(state.cloudSettings.enabled) {
                    const fields = document.getElementById('cloudSyncFields');
                    if(fields) fields.classList.remove('d-none');
                }
                document.getElementById('firebaseApiKey').value = state.cloudSettings.apiKey || '';
                document.getElementById('firebaseProjectId').value = state.cloudSettings.projectId || '';
                document.getElementById('firebaseDbUrl').value = state.cloudSettings.dbUrl || '';
            }
        } catch (e) { console.warn('Cloud settings not found'); }
        ...
        try {
            const ds = await dbGetAll('data_storage_files');
            state.dataStorageFiles = ds || [];
        } catch (e) { state.dataStorageFiles = []; }
...
if (cloudSyncSwitch) {
    cloudSyncSwitch.addEventListener('change', (e) => {
        const fields = document.getElementById('cloudSyncFields');
        if (e.target.checked) {
            ...
            // Disable sync
            state.cloudSettings.enabled = false;
            dbPut('settings', { key: 'cloudSettings', value: state.cloudSettings });
            location.reload(); // Reload to stop listeners
        }
    });
}
...
if (btnConnectCloud) {
    btnConnectCloud.addEventListener('click', async () => {
        ...
        try {
            await dbPut('settings', { key: 'cloudSettings', value: state.cloudSettings });
            alert('Konfigurasi disimpan! Menghubungkan ke Cloud...');
            initCloudSync();
        } catch (e) {
...
document.getElementById('saveMultiSkillBtn').addEventListener('click', async () => {
    ...
    try {
        ...
        // Refresh Personal Karyawan if visible
        if (typeof currentPersonalEmpId !== 'undefined' && currentPersonalEmpId === currentMultiSkillEmpId) {
            renderPersonalKaryawan(currentPersonalEmpId);
        }
    } catch (err) {
        console.error(err);
...
document.getElementById('saveEvalBtn').addEventListener('click', async () => {
    ...
    try {
        ...
        // Refresh Personal Karyawan if visible
        if (typeof currentPersonalEmpId !== 'undefined' && currentPersonalEmpId === currentEvalEmpId) {
            renderPersonalKaryawan(currentPersonalEmpId);
        }
    } catch (err) {
        console.error(err);
...
window.cycleTanokoSkill = async (empId, model, currentScore) => {
    ...
    // Refresh chart if the employee is currently being viewed in Personal Karyawan
    if (typeof currentPersonalEmpId !== 'undefined' && currentPersonalEmpId === empId) {
        renderMultiSkillByPartChart(empId);
        renderPersonalKaryawan(empId);
    }
...
function showSection(sectionId) {
    const sections = [
        'dashboardSection', 
        'personalKaryawanSection', 
        'dataFileStorageSection'
    ];
    ...
    // Map sectionId to buttonId
    const sectionToBtn = {
        'dashboardSection': 'btnDashboard',
        'personalKaryawanSection': 'btnPersonalKaryawan',
        'dataFileStorageSection': 'btnDataFileStorage'
    };

    const btnId = sectionToBtn[sectionId];
    if (btnId) {
...
// Update existing listeners to use showSection
...
if (btnPersonalKaryawan) {
    btnPersonalKaryawan.addEventListener('click', (e) => {
        e.preventDefault();
        showSection('personalKaryawanSection');
        populatePkSearch();
    });
}

// Data Storage listener
const btnDataFileStorage = document.getElementById('btnDataFileStorage');
if (btnDataFileStorage) {
    btnDataFileStorage.addEventListener('click', (e) => {
        e.preventDefault();
        showSection('dataFileStorageSection');
        renderDataFileStorage();
    });
}

// ==========================================
// 16. DATA STORAGE FUNCTIONS
// ==========================================

function renderDataFileStorage() {
    renderFileColumn('record_training', 'listRecordTraining', 'searchRecordTraining');
    renderFileColumn('training_multiskill', 'listTrainingMultiSkill', 'searchTrainingMultiSkill');
    renderFileColumn('evaluasi_multiskill', 'listEvaluasiMultiskill', 'searchEvaluasiMultiskill');
    renderFileColumn('record_new_mp', 'listRecordNewMP', 'searchRecordNewMP');
}

function renderFileColumn(category, listId, searchId) {
    ...
    // Combine data from dataStorageFiles and other relevant sources if any
    let files = state.dataStorageFiles.filter(f => f.category === category);

    // Apply search filter
    if (searchTerm) {
        files = files.filter(f => {
            const date = f.date || f.uploadDate || '';
            const name = f.trainingName || f.empName || f.fileName || '';
            const trainer = f.trainer || '';
            const model = f.model || '';
            const nik = f.nik || '';
            ...
            return date.toLowerCase().includes(searchTerm) || 
                   ...
                   trainer.toLowerCase().includes(searchTerm) ||
                   model.toLowerCase().includes(searchTerm) ||
                   nik.toLowerCase().includes(searchTerm) ||
    ...
    if (files.length === 0) {
        listElement.innerHTML = '<li class="text-center text-muted small p-3">Tidak ada data</li>';
        return;
    }
    ...
    files.forEach(file => {
        ...
        // Format date for display (e.g., "12 Maret 2026")
        let displayDate = file.date || file.uploadDate || '-';
        ...
        let metaInfo = '';
        if (file.trainingName) metaInfo += `<strong>${file.trainingName}</strong>`;
        if (file.empName) metaInfo += `${metaInfo ? ' - ' : ''}${file.empName}`;
        if (file.trainer) metaInfo += `<br><small>Trainer: ${file.trainer}</small>`;
        if (file.model) metaInfo += `<br><small>Model: ${file.model} (Lvl ${file.level || '-'})</small>`;
        if (file.result) metaInfo += `<br><small>Hasil: <span class="badge bg-${file.result === 'OK' ? 'success' : 'danger'}">${file.result}</span></small>`;
        if (file.status) metaInfo += `<br><small>Status: ${file.status}</small>`;
        if (!metaInfo) metaInfo = file.fileName || 'Tanpa keterangan';

        li.innerHTML = `
            <div class="pdf-icon-box" onclick="viewPDF('${file.fileData}')">
                <i class="fas fa-file-pdf"></i>
            </div>
            <div class="storage-item-info" onclick="viewPDF('${file.fileData}')">
                <div class="display-date">${displayDate}</div>
                <div class="meta-info">${metaInfo}</div>
            </div>
            <div class="storage-item-actions">
                <button class="btn btn-xs btn-outline-primary" onclick="viewPDF('${file.fileData}')">
                    <i class="fas fa-eye"></i> View
                </button>
                <button class="btn btn-xs btn-danger" onclick="deleteDataStorageFile('${file.id}', '${category}')">
...
['searchRecordTraining', 'searchTrainingMultiSkill', 'searchEvaluasiMultiskill', 'searchRecordNewMP'].forEach(id => {
    const el = document.getElementById(id);
    if (el) {
        el.addEventListener('input', () => {
            const categoryMap = {
                'searchRecordTraining': 'record_training',
                'searchTrainingMultiSkill': 'training_multiskill',
                'searchEvaluasiMultiskill': 'evaluasi_multiskill',
                'searchRecordNewMP': 'record_new_mp'
            };
            const listMap = {
                'searchRecordTraining': 'listRecordTraining',
                'searchTrainingMultiSkill': 'listTrainingMultiSkill',
                'searchEvaluasiMultiskill': 'listEvaluasiMultiskill',
                'searchRecordNewMP': 'listRecordNewMP'
            };
            renderFileColumn(categoryMap[id], listMap[id], id);
        });
    }
});

function openAddDataStorageModal(category) {
    const modalTitle = document.getElementById('addDataStorageTitle');
    const categoryInput = document.getElementById('dsCategory');
    const specificFields = document.getElementById('dsSpecificFields');
    const commonFields = document.getElementById('dsCommonFields');
    const form = document.getElementById('dataStorageForm');
    
    form.reset();
    categoryInput.value = category;
    specificFields.innerHTML = '';
    
    // Default show common fields (NIK/Name)
    commonFields.classList.remove('d-none');

    switch(category) {
        case 'record_training':
            modalTitle.innerText = 'Tambah PDF Record Training';
            commonFields.classList.add('d-none'); // Record training doesn't necessarily need a specific employee
            specificFields.innerHTML = `
                <div class="mb-3">
                    <label class="form-label">Nama Training</label>
                    <input type="text" id="dsTrainingName" class="form-control" required>
                </div>
                <div class="mb-3">
                    <label class="form-label">Tanggal</label>
                    <input type="date" id="dsDate" class="form-control" required>
                </div>
                <div class="mb-3">
                    <label class="form-label">Trainer</label>
                    <input type="text" id="dsTrainer" class="form-control" required>
                </div>
            `;
            break;
        case 'training_multiskill':
            modalTitle.innerText = 'Tambah PDF Training Multiskill';
            specificFields.innerHTML = `
                <div class="mb-3">
                    <label class="form-label">Model</label>
                    <select id="dsModel" class="form-select" required>
                        ${modelsList.map(m => `<option value="${m}">${m}</option>`).join('')}
                    </select>
                </div>
                <div class="mb-3">
                    <label class="form-label">Level</label>
                    <select id="dsLevel" class="form-select" required>
            ...
            `;
            break;
        case 'evaluasi_multiskill':
            modalTitle.innerText = 'Tambah PDF Evaluasi Multiskill';
            specificFields.innerHTML = `
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <select id="dsResult" class="form-select" required>
            ...
            break;
        case 'record_new_mp':
            modalTitle.innerText = 'Tambah PDF Record New MP';
            specificFields.innerHTML = `
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <select id="dsStatus" class="form-select" required>
    ...
    addDataStorageModal.show();
}

// Listener for NIK autocomplete in Data Storage Modal
const dsNikInput = document.getElementById('dsNik');
if (dsNikInput) {
    dsNikInput.addEventListener('input', (e) => {
        const val = e.target.value;
        const emp = state.employees.find(ev => ev.nik === val || ev.name === val);
        if (emp) {
            document.getElementById('dsNik').value = emp.nik;
            document.getElementById('dsEmpName').value = emp.name;
        } else {
            document.getElementById('dsEmpName').value = '';
        }
    });
}

const saveDataStorageBtn = document.getElementById('saveDataStorageBtn');
if (saveDataStorageBtn) {
    saveDataStorageBtn.addEventListener('click', async () => {
        const category = document.getElementById('dsCategory').value;
        const nik = document.getElementById('dsNik').value;
        const empName = document.getElementById('dsEmpName').value;
        ...
        reader.onload = async (e) => {
            ...
            if (document.getElementById('dsTrainingName')) data.trainingName = document.getElementById('dsTrainingName').value;
            if (document.getElementById('dsDate')) data.date = document.getElementById('dsDate').value;
            if (document.getElementById('dsTrainer')) data.trainer = document.getElementById('dsTrainer').value;
            if (document.getElementById('dsModel')) data.model = document.getElementById('dsModel').value;
            if (document.getElementById('dsLevel')) data.level = document.getElementById('dsLevel').value;
            if (document.getElementById('dsResult')) data.result = document.getElementById('dsResult').value;
            if (document.getElementById('dsStatus')) data.status = document.getElementById('dsStatus').value;

            try {
                await dbPut('data_storage_files', data);
                state.dataStorageFiles.push(data);
                
                // Re-render the storage view
                renderDataFileStorage();

                addDataStorageModal.hide();
...
async function deleteDataStorageFile(id, category) {
    if (confirm('Apakah Anda yakin ingin menghapus file ini?')) {
        try {
            await dbDelete('data_storage_files', id);
            state.dataStorageFiles = state.dataStorageFiles.filter(f => f.id !== id);
...
function viewPDF(base64) {
    const container = document.getElementById('certPreviewContainer');
    if (!container) return;
    container.innerHTML = `<iframe src="${base64}" width="100%" height="500px"></iframe>`;
    certModal.show();
}
...
// ==========================================
// 19. PDF EXPORT (EMPLOYEE TRAINING)
// ==========================================
...
function openPkMultiSkillEditModal(empId) {
    try {
        ...
        if (!tbody) {
            console.error('Element pkMultiSkillEditBody not found');
            return;
        }
        tbody.innerHTML = '';
        ...
        console.error('Error opening multi skill edit modal:', error);
    ...
    pkMultiSkillSaveBtn.addEventListener('click', async () => {
        try {
            ...
            modelsList.forEach((model, idx) => {
                ...
                if (!tEl || !aEl) {
                    console.error(`Element for model ${model} not found`);
                    return;
                }
            ...
            console.error('Error saving multi skill data:', error);
...
function renderPersonalKaryawan(empId) {
    ...
    if (emp.trainings && emp.trainings.length > 0) {
        emp.trainings.forEach(t => {
            const row = document.createElement('tr');
            const isComplete = !!(t.materi && t.date && t.duration && t.trainer);
    ...
    if (tanokoData && tanokoData.skills) {
        ...
        if (scores.length > 0) {
            ...
            avg = Math.round(total / modelsList.length); // Always divide by total models for coverage percentage
        }
    }
...
function renderMultiSkillByPartChart(empId) {
    ...
    modelsList.forEach(model => {
        const score = tanokoData.skills?.[model] ?? 0;
        const partData = msRec?.data?.[model];

        const target = partData?.target ?? 20;
        const actual = partData?.actual ?? Math.round((score / 100) * target);
...
window.deleteAttendance = deleteAttendance;
window.viewAttendanceFile = viewAttendanceFile;
window.editPkModelScore = editPkModelScore;
window.handleModelFileChange = handleModelFileChange;
window.editReportData = editReportData;
window.deleteReportData = deleteReportData;
window.viewPDF = viewPDF;
window.deleteDataStorageFile = deleteDataStorageFile;
window.openAddDataStorageModal = openAddDataStorageModal;
window.openReport = () => { renderReportTable(); reportModal.show(); };
window.openAddReport = () => { resetReportForm(); document.getElementById('inputReportTitle').innerText = 'Input Data Report Training'; reportModal.hide(); inputReportModal.show(); };
